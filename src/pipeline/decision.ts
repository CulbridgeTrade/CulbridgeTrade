...
import { Violation, ValidationResult, Decision, Confidence, Source } from "../types";
...
function buildBlockResult(violations: Violation[], source: Source): ValidationResult {
...
function buildWarningResult(violations: Violation[], source: Source): ValidationResult {
  const top = violations[0];
  const reasons = violations.map(v => v.description);
  const actions = buildActions(violations);
...
function buildOKResult(source: Source, violations: Violation[]): ValidationResult {
  return {
    decision: "OK",
...
function buildActions(violations: Violation[]): string[] {
  ...
  if (types.includes("MISSING_REQUIRED_DOCUMENT")) {
    const missing = violations
      ...
      .flatMap(v => (v.data.missing as string[]) || []);
...
function calculateOverallConfidence(violations: Violation[]): Confidence {
  const confidences = violations.map(v => v.confidence);
  
  // If any is LOW, overall is LOW
  if (confidences.includes("LOW")) return "LOW";
  
  // If any is MEDIUM, overall is MEDIUM
  if (confidences.includes("MEDIUM")) return "MEDIUM";
  
  return "HIGH";
}

export default { buildDecision };
